...
@section('content_header')
    <h1 class="m-0 text-dark">{{ isset($cliente) ? 'Editar Cliente' : 'Novo Cliente' }}</h1>
@stop

@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">{{ isset($cliente) ? 'Atualizar dados do cliente' : 'Cadastro de novo cliente' }}</h3>
        </div>
        ...
        <form action="{{ isset($cliente) ? route('clientes.update', $cliente->id) : route('clientes.store') }}" method="POST">
            ...
            <div class="card-footer">
                <div class="row w-100">
                    <div class="col-md-6">
                        <a href="{{ route('clientes.index') }}" class="btn btn-secondary btn-block shadow">
                            <i class="fas fa-arrow-left mr-1"></i> Cancelar
                        </a>
                    </div>
                    <div class="col-md-6 text-right">
                        <button type="submit" class="btn btn-primary btn-block shadow">
                            <i class="fas fa-save mr-1"></i> {{ isset($cliente) ? 'Atualizar' : 'Salvar' }}
                        </button>
                    </div>
                </div>
            </div>
